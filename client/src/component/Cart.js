...
export default function Cart({isDisplayed, itemsInCart, setIsCartDisplayed, setItemsInCart}) {
    ...
    const handleRemove = (id) => {
      setItemsInCart(prevItems => prevItems.filter(item => item.product.id !== id));
    }
                ...
                <h2 className="text-xl font-semibold mb-4">Your Cart</h2>
                    {itemsInCart?.length > 0 ? (
                      ...
                      {itemsInCart.map(item => (
                        <div key={item.product.id} className="mb-2 flex flex-row justify-between items-start relative">
                          <div className="h-14 w-14 bg-cover bg-center" style={{ backgroundImage: `url(${item.product.imageLink})` }} alt={item.product.altDescription}></div>
